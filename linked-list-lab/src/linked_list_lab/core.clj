...
(defn delete
  "Delete `elt` from `xx`."
  [elt xx]
  (let [state (delete-cons elt (:data xx))]
    (if (= (:data xx) state) (List. state (:size xx))
                        (List. state (- (:size xx) 1)))))
...
(defn delete-all
  ...
  [elt xx]
  (loop [yy (delete elt xx)
        zz xx]
      (if (= (:size yy) (:size zz)) yy
                                    (recur (delete elt yy) yy))
      )
)
